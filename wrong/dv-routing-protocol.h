...
public:
  ...
  void ProcessHelloReq(DVMessage lsMessage);   
  void ProcessHelloRsp(DVMessage lsMessage, Ipv4Address interface);
...
private:
  ...
  Timer m_neighborAuditTimer; 

  void BroadcastHello(); 

   //neighbors map
  struct HelloRoutingTableEntry
  {
  ...
  struct HelloNeighborTableEntry
  {
  Ipv4Address neighborAddr;
  Ipv4Address interfaceAddr;
  };

  std::map<uint32_t, HelloNeighborTableEntry> m_helloNeighborTable;
  std::map<uint32_t, HelloRoutingTableEntry> m_helloRoutingTable;
};
